...
export const Cursor = ({ appbarRef }) => {
   ...
   const smoothMouse = {
      x: useSpring(mouse.x, { damping: 100, stiffness: 1000, mass: 0.5 }),
      y: useSpring(mouse.y, { damping: 100, stiffness: 1000, mass: 0.5 }),
   };
   ...
   return (
      <motion.div
         className={`w-[20px] h-[20px] fixed top-0 left-0 rounded-full ${
            isHovered
               ? "z-0 bg-[#727171] opacity-[30%]"
               : "z-10 bg-[#b5b5b5] opacity-[50%]"
         }`}
         style={{
            left: smoothMouse.x,
            top: smoothMouse.y,
